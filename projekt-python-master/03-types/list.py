...
print('\n1. Cvičení\n***********************************************************************************************')
print(f'\tVypíše poslední 2 prvky ze seznamu numbers: {numbers[4:]}')
...
# key = navratova hodnat z funkce(item) return item[0]
...
print('\n2. Cvičení\n***********************************************************************************************')
hundreds = [number for number in list(range(1, 2000)) if number % 200 == 0]
print(f'a) Cisla v rozsahu 1-2000 delitelna 200 beze zbytku: {hundreds}')

Ascii = []                                                   # Tahle varianta funguje,
for i in range(50): Ascii.append(chr(randint(65, 90)))       # ale na vice radcich.

Ascii_one_letter = list( [letter for letter in list(chr(randint(65, 90)))] ) # tahle varianta by chtěla projet 50x v cykle
print(f'b) {Ascii}')

sliced_hundreds = hundreds[3:6]
print(f'c) hundreds bez prvnich a posledních tri prvku {sliced_hundreds}')

#uniq = []
#for asciiLetter in Ascii:
#    if asciiLetter not in uniq:
#        uniq.append(asciiLetter)

uniq = set(Ascii)
print(f'd) jedinecne hodnoty za pomoci mnoziny set(Ascii){uniq}')

combine = [(sliced_hundreds[0], Ascii[:len(sliced_hundreds)][0]), (sliced_hundreds[1], Ascii[:len(sliced_hundreds)][1]), (sliced_hundreds[2], Ascii[:len(sliced_hundreds)][2])]
print(f'e) seznam n-tic combine: {combine}')
print(f'***********************************************************************************************')
...
persons += [('Dagmar', 5, 'žena'), ('Krystian', 65, 'muž'), ('Alex', 17, 'muž'), ('Jitka', 23, 'žena')]

woman = list(filter(lambda item: item[2] == "žena", persons))
print("Zeny: ", woman)

ipeople = [i for i in persons if "i" in i[0]]

ipeople.sort(key=lambda item: item[2])
print("Lide majici \"i\" ve jmene", ipeople)

csv = ""
for i in range(len(ipeople)):
    csv += str(i+1) + ";" + str(ipeople[i])[2:-2].replace("'", "").replace(", ", ";") + ";"
print("CSV: ", csv)
print(f'***********************************************************************************************')
